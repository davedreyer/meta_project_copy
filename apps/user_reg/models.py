...
class UserManager(models.Manager):
	def reg_check(self, postData):
		...
		if len(postData['first']) < 2:
			error_messages.append('First name must contain at least 2 letters!')
			
		if not re.search(name_match, postData['first']):
			error_messages.append('First name must have only letters!')
			
		if len(postData['last']) < 2:
			error_messages.append('Last name is too short!')
		...
		if not postData['confirm_password']:
			error_messages.append("Confirm password must contain at least 1 capital letter and 1 number!")
		...
		if error_messages:
			...
			new_user = User.objects.create(email=postData['email'],first_name=postData['first'],last_name=postData['last'],pw_hash=pw_hash)
